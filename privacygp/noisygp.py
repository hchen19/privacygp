...
class NoisyGP(torch.nn.Module):
    """
    Gaussian Process implementation that includes noise in the training data.
    """
    ...
    def __init__(
        ...
        """
        Initialize the ExcludingGP model with training data and parameters.
    ...
    def train(self):
        """Train the GP model on training data points."""
    ...
    def test(self, test_x):
        ...
        with torch.no_grad():
            ...
            self.test_mean = preds.mean
            self.test_cov = preds.covariance_matrix

            # Denormalize the mean and variance if normalization was applied
            if self.params['normalize']:
                self.test_mean = self.test_mean * self.params['train_y_std'] + self.params['train_y_mean']
                self.test_cov = self.test_cov* (self.params['train_y_std'] ** 2)
            
            return self.test_mean, self.test_cov

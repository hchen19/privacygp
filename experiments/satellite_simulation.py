...
def get_satellite_data():
    ...
    data_path = os.path.join(data_dir, 'satellite_adata.csv')
...
def run_multiple_seeds(
    ...
    aggregate_path = os.path.join(results_dir, 'satellite_aggregate_metrics.pkl')
...
def save_metrics_to_csv(aggregate_metrics, algs, alphas, epsilons, output_file='satellite_metrics.csv'):
    """
...
def plot_privacy_results(
    ...
    for alpha in alphas:
        ...
        fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, -0.02), 
                   ncol=min(3, len(handles)), fontsize=20)
        
        # Set title
        fig.suptitle(f'Privacy-aware GP (H={alpha}K)', color='blue', fontsize=24, fontweight='bold', y=1.05)
        
        # Save figure
        plt.tight_layout()
        plt.subplots_adjust(top=0.90, bottom=0.33)
        fig_path = os.path.join(figs_dir, f'satellite_pa_alpha_{alpha}_seed_{seed}.png')
    ...
    if dp_algs or 'exclude' in algs:  # Add 'exclude' check here
        # Create figure with subplots
        fig, axes = plt.subplots(1, n_components, figsize=(5*n_components, 5))
        if n_components == 1:
            axes = [axes]
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.90, bottom=0.33)
        fig_path = os.path.join(figs_dir, f'satellite_dp_all_eps_seed_{seed}.png')
        plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    ...
    if dp_algs or 'exclude' in algs:  # Add 'exclude' check here
        ...
        fig_path = os.path.join(figs_dir, f'satellite_dp_all_eps_seed_{seed}_with_zoom.png')
        plt.savefig(fig_path, dpi=300, bbox_inches='tight')
